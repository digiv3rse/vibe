import { tw, useCopyToClipboard } from '@vibe/browser'
import { VIBE_APP_NAME, VIBE_EMBED_URL } from '@vibe/constants'
import { EVENTS, Tower } from '@vibe/generic'
import { CodeOutline, CopyOutline, Modal, Tooltip } from '@vibe/ui'
...
const EmbedMedia: FC<Props> = ({ publicationId, isAudio }) => {
  ...
  let iframeCode = `<iframe width="560" height="315" src="${VIBE_EMBED_URL}/${publicationId}?autoplay=1&t=0&loop=0" title="${VIBE_APP_NAME} player" frameborder="0" allow="accelerometer; autoplay; clipboard-write;" allowfullscreen></iframe>`

  if (isAudio) {
    iframeCode = `<iframe width="100%" height="300" src="${VIBE_EMBED_URL}/${publicationId}" title="${VIBE_APP_NAME} player" frameborder="0"></iframe>`
  }
        ...
        <div className="flex flex-col space-y-3">
          <div className="w-full overflow-hidden rounded">
            <iframe
              ...
              src={`${VIBE_EMBED_URL}/${publicationId}`}
              title={`${VIBE_APP_NAME} player`}
