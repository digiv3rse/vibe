...
import { Button, Callout, Spinner, Tooltip, UserOutline } from '@vibe/ui'
...
const CollectPublication: FC<Props> = ({ publication, action }) => {
  ...
  // Collect via DiGi Manager
  const legacyCollectViaDiGiManager = async (request: LegacyCollectRequest) => {
    ...
    if (
      !data?.legacyCollect ||
      data?.legacyCollect.__typename === 'DiGiProfileManagerRelayError'
    ) {
  ...
  const collectNow = async () => {
    ...
    if (isLegacyCollectModule) {
      ...
      if (canUseDiGiManager && isFreeForAnyone) {
        return await legacyCollectViaDiGiManager(legacyCollectRequest)
      }
