...
import type { ProfileManagersRequest } from '@vibe/digi'
import {
...
import { Button, ExternalOutline, Input, Modal, Spinner } from '@vibe/ui'
...
const Managers = () => {
  ...
  const profileManagersWithoutDiGiManager = useMemo(() => {
    if (!data?.profileManagers?.items) {
      return []
    }
    return data.profileManagers.items.filter((m) => !m.isDiGiManager)
  ...
  const addManager = async ({ address }: FormData) => {
    ...
    setSubmitting(true)
    return await toggleDiGiManager({
      variables: {
  ...
  return (
    <div>
      ...
      <div className="mt-3">
        ...
        {(!loading && !profileManagersWithoutDiGiManager?.length) || error ? (
          <NoDataFound withImage isCenter />
        ) : null}
        {profileManagersWithoutDiGiManager?.length ? (
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {profileManagersWithoutDiGiManager?.map(({ address }) => (
              <Entry
                key={address}
                address={address}
