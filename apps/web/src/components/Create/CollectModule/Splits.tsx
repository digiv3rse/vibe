...
import { tw, useDebounce, useOutsideClick } from '@vibe/browser'
import {
  DIGI_CUSTOM_FILTERS,
...
import type { Profile, RecipientDataInput } from '@vibe/digi'
import { LimitType, useSearchProfilesLazyQuery } from '@vibe/digi'
import {
  getProfile,
  getProfilePicture,
  splitNumber,
...
import { InfoOutline, Input, Spinner, TimesOutline, Tooltip } from '@vibe/ui'
...
const Splits: FC<Props> = ({ submitContainerRef }) => {
  ...
  const isIncludesDonationAddress =
    splitRecipients.filter((el) => el.recipient === VIBE_ADMIN_ADDRESS).length >
    0
  ...
  const addDonation = () => {
    const splits = splitRecipients
    splits.push({ recipient: VIBE_ADMIN_ADDRESS, split: 2 })
    setSplitRecipients([...splits])
    scrollToSubmit()
  }
      ...
      {splitRecipients.map((splitRecipient, i) => (
        <div className="flex gap-1.5" key={i}>
          <div className="relative w-full">
            <Input
              placeholder={`0x12345...89 or ${DIGI_NAMESPACE_PREFIX}tape`}
              ...
              spellCheck="false"
              title={
                splitRecipient.recipient === VIBE_ADMIN_ADDRESS
                  ? VIBE_APP_NAME
                  : undefined
              }
              suffix={
                splitRecipient.recipient === VIBE_ADMIN_ADDRESS
                  ? `${VIBE_APP_NAME}.xyz`.toLowerCase()
                  : ''
              }
              disabled={splitRecipient.recipient === VIBE_ADMIN_ADDRESS}
