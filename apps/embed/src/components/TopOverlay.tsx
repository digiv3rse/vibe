import { useCopyToClipboard } from '@vibe/browser'
...
import { CopyOutline } from '@vibe/ui'
...
const TopOverlay: FC<OverlayProps> = ({ playerRef, video }) => {
  ...
  const onCopyVideoUrl = async () => {
    await copy(`${VIBE_WEBSITE_URL}/watch/${video.id}`)
    ...
    >
        ...
        <div className="flex items-center justify-between space-x-6 bg-gradient-to-b from-black/80 via-black/30 to-transparent p-3.5 pb-6">
          <div className="flex flex-1 items-center">
            <Link
              ...
              href={`${VIBE_WEBSITE_URL}/u/${getProfile(video.by)?.slug}`}
              target="_blank"
              onClick={() =>
            ...
            <div className="flex flex-col">
              <Link
                ...
                href={`${VIBE_WEBSITE_URL}/watch/${video?.id}`}
                target="_blank"
                onClick={() =>
      ...
      <div className="absolute bottom-2 right-0 md:bottom-4">
        <Link
          ...
          title={`Watch on ${VIBE_APP_NAME}`}
          href={`${VIBE_WEBSITE_URL}/watch/${video?.id}`}
          target="_blank"
          onClick={() => Tower.track(EVENTS.EMBED_VIDEO.CLICK_WATCH_ON_VIBE)}
        >
          <img
